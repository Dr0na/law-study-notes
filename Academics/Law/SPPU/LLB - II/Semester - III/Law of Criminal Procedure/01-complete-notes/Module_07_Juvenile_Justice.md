...
```mermaid
graph TD
    A["Child (Person < 18 years)"] --> B["Child in Conflict with Law (CCL)<br/>Section 2(13)"]
    A --> C["Child in Need of Care and Protection (CNCP)<br/>Section 2(14)"]
    
    B --> D["Juvenile Justice Board (JJB)<br/>Section 4"]
    C --> E["Child Welfare Committee (CWC)<br/>Section 27"]
    
    D --> F["Observation Home / Special Home<br/>Rehabilitation & Reform"]
    E --> G["Children's Home / Foster Care<br/>Care & Protection"]
```
...
```mermaid
graph TD
    A["CCI / Agency identifies Orphan/Abandoned/Surrendered Child"] --> B["CWC conducts inquiry & declares child 'Legally Free for Adoption'"]
    B --> C["Prospective Adoptive Parents (PAPs) register on CARA Portal"]
    C --> D["Home Study Report of PAPs conducted by Special Agency"]
    D --> E["CARA matches Child with PAPs based on preferences"]
    E --> F["DM / ADM issues final Adoption Order (Post-2021 Amendment)"]
```
